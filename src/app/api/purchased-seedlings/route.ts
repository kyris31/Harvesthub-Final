  ...
  try {
    ...
    const [newSeedling] = await db
      ...
      .values({
        ...
        purchaseDate: validatedData.purchaseDate || new Date().toISOString().slice(0, 10),
        quantityPurchased: parseInt(validatedData.quantityPurchased, 10),
        currentQuantity: parseInt(validatedData.quantityPurchased, 10),
