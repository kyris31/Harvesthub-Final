...
type BroilerFlockSummary = Awaited<ReturnType<typeof getAllBroilerFlocksSummary>>[number]

interface BroilerReportClientProps {
  initialData: BroilerFlockSummary[]
  initialStartDate: string
  initialEndDate: string
}

export default function BroilerReportClient({
  initialData,
  initialStartDate,
  initialEndDate,
}: BroilerReportClientProps) {
  const [flocks, setFlocks] = useState(initialData)

  const handleFilterChange = async (startDate: string, endDate: string) => {
    const updated = await getAllBroilerFlocksSummary(startDate, endDate)
    setFlocks(updated)
  }
      ...
      {flocks.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="mb-2 text-xl font-semibold">No broiler flocks in this period</h3>
            <p className="text-muted-foreground mb-6 max-w-md">
              Adjust the date range above, or create a flock with purpose set to "Broilers" to start
