...
type ProfitabilityData = Awaited<ReturnType<typeof getProfitabilityReport>>

interface ProfitabilityReportClientProps {
...
export default function ProfitabilityReportClient({
  ...
  initialStartDate,
  initialEndDate,
}: ProfitabilityReportClientProps) {
  ...
  const handleFilterChange = async (startDate: string, endDate: string) => {
    const updated = await getProfitabilityReport(startDate, endDate)
    setReport(updated)
  }
